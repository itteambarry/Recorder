...
async function startCamera() {
  try {
    // Ask for camera + mic (portrait‐friendly)
    const constraints = {
      audio: true,
      video: {
        facingMode: "user",
    ...
    // Assign the raw stream directly to <video>
    previewVideo.srcObject = mediaStream;
    previewVideo.play();

    // Hide “Start Camera” → reveal mode buttons + record button
...
async function recordFiveSeconds() {
  ...
  // 2) Turn on flash overlay if needed
  ...
  // 4) Set up MediaRecorder on the raw camera stream
  recordedChunks = [];
  let options = { mimeType: "video/mp4; codecs=avc1" };
  try {
    mediaRecorder = new MediaRecorder(mediaStream, options);
  } catch (e) {
    // Fallback if that MIME isn’t available
